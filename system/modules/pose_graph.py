...
class PoseGraph_Edge(object):
    ...
    def __init__(self, src_scan_token: int, dst_scan_token: int, SE3: torch.Tensor, information_mat: torch.Tensor, type: Literal['odom', 'loop', 'locz', 'prxy'], confidence=None, rmse=None) -> None:
        ...
        self.information_mat: torch.Tensor = detach_to_cpu(information_mat).reshape(6, 6) if information_mat is not None else None
...
class PoseGraph(object):
    ...
    def has_edge(self, src_token: int, end_token: int, _DEBUG_warning=True):
        exists = (src_token, end_token) in self.edge.keys()
        # __DEBUG__
        if (_DEBUG_warning and (exists == False) and ((end_token, src_token) in self.edge.keys())):
            logger.critical(f'has_edge: {(src_token, end_token)} not in edge, but {(end_token,src_token)} exits...')
            raise f'has_edge: {(src_token, end_token)} not in edge, but {(end_token,src_token)} exits...'
        return exists
        ...
        for scan in scan_packs:
            ...
            if (full_pcd == False):
                if (self.__global_map_cache[scan.token][0] is not None):
                    ...
                    points[-3:, :] = (r_scan @ points[-3:, :].float() + t_scan).type_as(points)
    ...
    def graph_search(self, token: int, neighbor_level: int, coor_sys: int, edge_type: Union[Literal['all'], List[str]] = 'all', max_k: Union[None, int] = 16):
        ...
        while (len(bfs) > 0 and (max_k == None or len(global_map_scan_list) < max_k)):
            level_remain, scanpack = bfs.pop(0)  # 取根结点
            # assert scanpack.coor_sys == coor_sys
            if (scanpack.token in global_map_scan_list.keys()):  # 当前节点已经搜索过，则跳过
                continue
            global_map_scan_list[scanpack.token] = scanpack  # 新搜索的根节点加入到搜索节点中
            if (level_remain <= 0):
                continue
            bfs += [(level_remain - 1, self.get_scanpack(t)) for t in self.get_neighbor_tokens(scanpack.token)
                    if ((self.has_edge(scanpack.token, t, _DEBUG_warning=False) and self.get_edge(scanpack.token, t).type in edge_type) or (
                        self.has_edge(t, scanpack.token, _DEBUG_warning=False) and self.get_edge(t, scanpack.token).type in edge_type))]
    ...
    def shortest_path_length(self, src_scan_token, dst_scan_token, edge_type: Union[Literal['all'], List[str]] = 'all', infinity_length: int = 50):
        ...
        while (len(bfs) > 0):
            distance, scanpack = bfs.pop(0)  # 取根结点
            if (scanpack.token == dst_scan_token):
                return distance
            if (scanpack.token in vis):  # 当前节点已经搜索过，则跳过
                continue
            vis.add(scanpack.token)  # 新搜索的根节点加入到搜索节点中
