...
class OdometryThread():
    ...
    def search_candidates(self, new_scan: ScanPack) -> List[ScanPack]:
        ...
        topk_key_frame_dist, topk_keyframe_index = torch.topk(key_frame_distances.float(), dim=0, k=min(len(key_frames), self.args.odometer_candidates_num), largest=False)
    ...
    def odometry(self, new_scan: ScanPack, candidates: List[ScanPack]) -> List[PoseGraph_Edge]:
        ...
        for nearest_scan in candidates:
            ...
            global_dict['src_SE3'] = nearest_scan.SE3_gt
            global_dict['dst_SE3'] = new_scan.SE3_gt
            global_dict['src_pcd'] = nearest_scan.full_pcd
            global_dict['dst_pcd'] = new_scan.full_pcd
