...
class ResultLogger():
    ...
    def draw_trajectory(self, file_name: str = 'traj_jpg', draft: bool = False):
        ...
        for scan in self.posegraph_map.get_all_scans():
            ...
            if (scan.full_pcd is not None):
                ...
                full_points[:3, :] = (view_R @ (R_global @ (scan.full_pcd[:3, :].float()) + T_global) + view_T).type_as(full_points)  # (xyz+others, N)
                global_fullcloud_list.append(full_points)
            if (scan.key_points is not None):
                key_points = scan.key_points.clone()
                key_points[-3:, :] = (view_R @ (R_global @ (scan.key_points[-3:, :].float()) + T_global) + view_T).type_as(key_points)  # (fea+xyz, N)
    ...
    def save_map(self, file_name: str = 'map'):
        ...
        for scan in self.posegraph_map.get_all_scans():
            ...
            if (scan.full_pcd is not None):
                full_points = scan.full_pcd.clone()
                full_points[:3, :] = (R_global @ (scan.full_pcd[:3, :].float()) + T_global).type_as(full_points)  # (xyz+others, N)
                global_fullcloud_list.append(full_points)
            if (scan.key_points is not None):
                key_points = scan.key_points.clone()
                key_points[-3:, :] = (R_global @ (scan.key_points[-3:, :].float()) + T_global).type_as(key_points)  # (fea+xyz, N)
                global_keypoint_list.append(key_points)
