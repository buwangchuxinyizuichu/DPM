...
class LoopThread():
    ...
    def loop_closure_detection(self, new_scan: ScanPack, targets: Literal['self', 'others', 'all'] = 'all') -> List[ScanPack]:
        ...
        '''
        回环触发基本规则
        1. 图上距离不低于trust_range，即不在trust_zone_range1内 -> 限制过近的回环
        2. 图上距离在 1x 到 10x trust_range 且 转向角 < rotation_min 时不回环 -> 图上距离和角度需要至少有一个变化较大
        3. 图上距离在 1x 到 10x trust_range 且 欧式距离 < transaction_min 时不回环 -> 图上距离和欧式距离需要至少有一个变化较大
        4. 图上距离在 1x 到 10x trust_range 且 距离上个回环点欧式距离 < transaction_gap 时不回环 -> 限制连续的弯道回环
        '''
